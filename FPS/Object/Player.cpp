	...
	if (kabeNum != 0)
	{
		...
		if (isMoving_)
		{
			...
			for (i = 0; i < kabeNum; i++)
			{
				...
				if (HitCheck_Capsule_Triangle(moveAfterPos, VAdd(moveAfterPos, VGet(0.0f, 300.0f, 0.0f)), 130.0f, poly->Position[0], poly->Position[1], poly->Position[2]) == FALSE) continue;
				...
				for (j = 0; j < kabeNum; j++)
				{
					...
					// 当たっていたらループから抜ける
					if (HitCheck_Capsule_Triangle(moveAfterPos, VAdd(moveAfterPos, VGet(0.0f, 300.0f, 0.0f)), 130.0f, poly->Position[0], poly->Position[1], poly->Position[2]) == TRUE) break;
				}
		...
		else
		{
			...
			for (i = 0; i < kabeNum; i++)
			{
				...
				if (HitCheck_Capsule_Triangle(moveAfterPos, VAdd(moveAfterPos, VGet(0.0f, 300.0f, 0.0f)), 130.0f, poly->Position[0], poly->Position[1], poly->Position[2]) == TRUE)
				{
					hitFlag = true;
					break;
				}
